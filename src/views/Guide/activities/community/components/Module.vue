    ...
    <div :style="{ 'border-color': getColor }" class="module__container">
      <div class="module__title">
        <div class="module__image rounded-circle">
          <v-icon :color="selectedColor" light x-large>mdi-discord</v-icon>
        </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function calculateTimer() {
      const adkIndex = studentDoc.value.data.adks.findIndex(obj => obj.name === 'community');
      if (
    ...
    if (userRole.value === 'participant' && !!props.studentDoc?.data.adks) {
      if (props.studentDoc.data.adks.findIndex(obj => obj.name === 'community') === -1) {
        props.studentDoc.data.adks.push({ name: 'community' });
      ...
      watch(
        ...
        newData => {
          if (
            newData.data.adks.findIndex((objdata: { name: string }) => objdata.name === 'community')
          ) {
            calculateTimer();
          } else {
            props.studentDoc.data.adks.push({ name: 'community' });
