    ...
    <div :style="{ 'border-color': getColor }" class="module__container">
      <div class="module__title">
        <div class="module__image rounded-circle">
          <v-icon :color="selectedColor" light x-large>mdi-account-supervisor-circle</v-icon>
        </div>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    function calculateTimer() {
      const adkIndex = studentDoc.value?.data.adks.findIndex(obj => obj.name === 'team');
      if (
    ...
    if (userRole.value === 'participant' && !!props.studentDoc?.data.adks) {
      if (props.studentDoc.data.adks.findIndex(obj => obj.name === 'team') === -1) {
        props.studentDoc.data.adks.push({ name: 'team' });
      ...
      watch(
        () => studentDoc.value,
        newData => {
          if (newData.data.adks.findIndex((objdata: { name: string }) => objdata.name === 'team')) {
            calculateTimer();
          } else {
            props.studentDoc.data.adks.push({ name: 'team' });
            props.studentDoc.update().then(() => {
