    ...
    <div :style="{ 'border-color': getColor }" class="module__container">
      <div class="module__title">
        <div class="module__image rounded-circle">
          <v-icon :color="selectedColor" light x-large>mdi-timetable</v-icon>
        </div>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    function calculateTimer() {
      const adkIndex = props.teamDoc.data.adks.findIndex(obj => obj.name === 'tinker');
      if (
    ...
    if (userRole.value === 'participant' && props.teamDoc?.data.adks !== undefined) {
      if (props.teamDoc?.data.adks.findIndex(obj => obj.name === 'tinker') === -1) {
        props.teamDoc.data.adks.push({ name: 'tinker' });
        console.log(props.teamDoc.data);
      ...
      watch(
        ...
        newData => {
          if (
            newData.data.adks.findIndex((objdata: { name: string }) => objdata.name === 'tinker')
          ) {
            calculateTimer();
          } else {
            props.teamDoc.data.adks.push({ name: 'tinker' });
