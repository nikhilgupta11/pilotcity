...
export default defineComponent({
  ...
  setup(props: any, ctx: any) {
    ...
    const getComponent = computed(() => `module-${page.currentPage.toLowerCase()}`);
    ...
    const timeline = computed(() => timelineData.events.slice().reverse());
    function comment() {
    ...
    function calculateTimer() {
      const adkIndex = studentDoc.value.data.adks.findIndex(obj => obj.name === 'train');
      if (
    ...
    if (userRole.value === 'participant' && !!props.studentDoc?.data.adks) {
      if (props.studentDoc.data.adks.findIndex(obj => obj.name === 'train') === -1) {
        props.studentDoc.data.adks.push({ name: 'train' });
      ...
      watch(
        ...
        newData => {
          if (
            newData.data.adks.findIndex((objdata: { name: string }) => objdata.name === 'train')
          ) {
            calculateTimer();
          } else {
            props.studentDoc.data.adks.push({ name: 'train' });
