    ...
    <div :style="{ 'border-color': getColor }" class="module__container">
      <div class="module__title">
        <div class="module__image rounded-circle">
          <v-icon :color="selectedColor" light x-large>mdi-elevator-passenger</v-icon>
        </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    props.getTeamDoc();
    const readOnly = computed(() => !!readonlyInjected.value || !!props.readonly);
    ...
    function calculateTimer() {
      const adkIndex = props.teamDoc.data.adks.findIndex(obj => obj.name === 'Pitch');
      if (
    ...
    if (userRole.value === 'participant' && props.teamDoc.data.adks !== undefined) {
      if (props.teamDoc.data.adks.findIndex(obj => obj.name === 'Pitch') === -1) {
        props.teamDoc.data.adks.push({ name: 'Pitch' });
      ...
      watch(
        ...
        newData => {
          if (
            newData.data.adks.findIndex((objdata: { name: string }) => objdata.name === 'Pitch')
          ) {
            // calculateTimer();
          } else {
            props.teamDoc.data.adks.push({ name: 'Pitch' });
