<template>
  ...
  <v-container class="module">
    ...
    <div :style="{ 'border-color': getColor }" class="module__container pb-0 mb-0">
      <div class="module__title">
        <div class="module__image rounded-circle">
          <v-icon :color="selectedColor" light x-large>mdi-book-open</v-icon>
        </div>
      ...
      <div class="module__body">
        ...
        <div style="display: none">{{ 'Version bumped ' }}</div>
      </div>
    </div>
          ...
          <v-timeline-item fill-dot class="white--text mb-12" :color="selectedColor" large>
            ...
            >
              <template v-slot:append>
                <v-btn :disabled="readonly" rounded class="mx-0" outlined depressed @click="comment"> Post </v-btn>
              </template>
            </v-text-field>
          </v-timeline-item>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    function calculateTimer() {
      const adkIndex = studentDoc.value.data.adks.findIndex(obj => obj.name === 'scope');
      if (
