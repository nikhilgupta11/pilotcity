...
<script lang="ts">
import { defineComponent, reactive, ref, set, toRefs } from '@vue/composition-api';
...
export default defineComponent({
  ...
  setup(_, ctx) {
    ...
    async function loadData() {
      ...
      state.students = studentDocs.map(studentDoc => {
        ...
        return {
          ...studentDoc,
          program: programDoc.programName,
          programAdks: programDoc.adks,
          defaultOffer: programDoc.adks.find(adk => adk.name === 'offer')?.offer?.[0],
          newOffer: {},
          ...
          controllable: programDoc.organizers.some(userId => userId.equals(getObjectId.value)),
          // make offer stuff
    ...
    async function resolveStudentApplication(
      ...
      try {
        const result: {
          statusCode: number;
          error?: string;
        } = await getUser.value?.callFunction('resolveStudentApplication', {
